...
async function reset() {
  ...
  const contributor = await prisma.user.create({
    data: { email: 'contributor@test.com', password: cPwd, name: '测试投稿方', role: 'CONTRIBUTOR' }
  });
  console.log('投稿方已创建');

  const uPwd = await bcrypt.hash('user123456', 10);
  await prisma.user.create({
    data: { email: 'user@test.com', password: uPwd, name: '测试用户', role: 'USER' }
  });
  console.log('普通用户已创建');
